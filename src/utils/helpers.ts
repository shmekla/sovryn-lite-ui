...
export const getToken = (token: TOKEN) =>
  tokens[getCurrentNetwork().id].find(item => item.id === token) as TokenType;

export const listLoanTokens = () => loans[getCurrentNetwork().id];

export const getLoanToken = (token: TOKEN) =>
  loans[getCurrentNetwork().id].find(
...
export const toLocaleNumber = (value: number | string, decimals: number) =>
